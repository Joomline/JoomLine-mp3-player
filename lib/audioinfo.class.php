...
class AudioInfo {
	...
	*/
    ...
    function getId3tags($file)
    {
        ...
        // Exit here on error
        if (isset($this->info['error'])
            || !(isset($this->info['tags'])
                && isset($this->info['tags']['id3v2'])
                && is_array($this->info['tags']['id3v2'])
                && count($this->info['tags']['id3v2']))
        ) {
            $tagsIsEmpty = true;
        }
        ...
        if(!$tagsIsEmpty)
        {
            $tags = array_merge($tags, $this->info['tags']['id3v2']);
        }

        $return = array();
        foreach($tags as $k => $v){
            $return[$k] = !empty($v[0]) ? $v[0] : '';
        }
